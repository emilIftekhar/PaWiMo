    ...
    def __init__(self, var, filenames, days, counties):
        ...
        for filename in filenames:
            try:
                ...
                d_idx = np.array([ws.index(w) for w in days]).reshape((-1,1))
...
class BaseModel(object):
    ...
    * interaction effects (functions of distance in time and space relative to each datapoint)
    ...
    def __init__(self, trange, counties, ia_effect_filenames, num_ia=16, model=None, include_ia=True, include_eastwest=False, include_demographics=True, include_temporal=False, orthogonalize=False):
