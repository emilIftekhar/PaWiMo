...
from src.geo_utils import jacobian_sq
...
def uniform_times_by_day(days, n=500):
    res = OrderedDict()
    for day in days:
        time_min = datetime.datetime.combine(isoweek.day(*day).monday(), datetime.time.min)
        time_max = datetime.datetime.combine(isoweek.day(*day).sunday(), datetime.time.max)
        res[day] = np.random.rand(n) * (time_max-time_min) + time_min
    return res

def uniform_times_by_day(days, n=100):
    res = OrderedDict()
    seconds_of_day = 60*60*24
    for day in days:
        res[day] = np.random.rand(n) * seconds_of_day + (seconds_of_day * day)
    return res
    ...
    def __call__(self, days, counties):
        ...
        for i,day in enumerate(days):
            for j,county in enumerate(counties):
                idx = ((days-self.time_horizon) <= self.data.index)*(self.data.index < day)
